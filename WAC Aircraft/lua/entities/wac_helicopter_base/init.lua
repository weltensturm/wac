...
function ENT:physicsUpdate(ph)
	...
	if IsValid(pilot) then
		self.controls.pitch = self:GetPLControl(pilot, WAC_AIR_LEANP, self.controls.pitch)
		self.controls.yaw = self.BackRotor and self:GetPLControl(pilot, WAC_AIR_LEANY, self.controls.yaw) or 0
		self.controls.roll = self:GetPLControl(pilot, WAC_AIR_LEANR, self.controls.roll)
