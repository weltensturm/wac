...
local explode_inv = function(sep,s) return explode(s,sep) end; -- Enhances garry's explode function

function string.TrimExplode(s,sep)
		...
		if(line ~= "") then -- Only add lines with contents (no commented lines)
			if(line:sub(1,1) == "[") then -- Holy shit, it's a node
				...
				if(cur_node == "") then
					...
					local data = explode_inv("=",line);
					-- This is needed, because garry missed to add a limit to explode_inv
