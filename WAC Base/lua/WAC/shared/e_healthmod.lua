...
wac.healthMod={
	b = CreateConVar("wac_healthmod", 1, FCVAR),
...
if SERVER then
	local function getVar(s)
		if !GetGlobalBool("wac_healthmod_override") then return GetConVar(s):GetFloat() end
	...
	wac.hook("PlayerDeathSound", "wac_damage_cdsnd_check", function()
		if wac.healthMod.sound:GetInt()==1 then
			return true
		end
	end)
	...
	wac.hook("EntityTakeDamage", "wac_healthmod_takedamage", function(ent,i,at,amt,dmginfo)
		if ent:IsPlayer() then
			wac.player(ent)
			ent.wac.lastDamaged=CurTime()
		end
		if getVar("wac_healthmod")==1 then
			if dmginfo:IsFallDamage() then
				local multi = ent:GetVelocity().z/550
				dmginfo:SetDamage(10)
				dmginfo:ScaleDamage(math.abs(multi*multi*getVar("wac_falldamage")*5))
				return
			end
			if  ent:IsNPC() then
				dmginfo:ScaleDamage(getVar("wac_damage_npc"))
			elseif ent:IsPlayer() then
				dmginfo:ScaleDamage(getVar("wac_damage_player"))
			end
		end
	end)
